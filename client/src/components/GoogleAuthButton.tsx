...
const GoogleAuthButton: FC<GoogleAuthButtonProps> = ({
  ...
  const handleGoogleAuth = async (e: MouseEvent<HTMLAnchorElement>) => {
    e.preventDefault();
    ...
    try {
      ...
      // No need to set success state here as the user will be redirected to Google
    } catch (error) {
      console.error(`Error signing ${mode === "sign-in" ? "in" : "up"} with Google:`, error);
