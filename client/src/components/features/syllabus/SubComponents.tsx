...
export const extractPercentage = (text: string): string | null => {
  // First clean the text by removing whitespace
  const cleanText = text.trim();

  // Match a number followed by one or more % signs at the end of the string
  const match = cleanText.match(/(\d+)\s*%+$/);

  // If no match found, try to find any number followed by %
  if (!match) {
    const fallbackMatch = cleanText.match(/(\d+)\s*%/);
    if (!fallbackMatch) return null;

    // Return just the number if the text already ends with %
    return cleanText.endsWith('%') ? fallbackMatch[1] : `${fallbackMatch[1]}%`;
  }

  // Return just the number if the text already ends with %
  return cleanText.endsWith('%') ? match[1] : `${match[1]}%`;
};

/**
 * Extracts date from text using regex
 */
export const extractDateFromText = (
  text: string
): { date: string; isApproximate: boolean } | null => {
  ...
  const dateRegex = new RegExp(
    `(${monthPattern})\\s+(\\d+)|(\\d+)(?:st|nd|rd|th)?\\s+(?:of\\s+)?(${monthPattern})`,
    'i'
  );
...
export const isWebsiteField = (text: string): boolean => {
  const urlRegex =
    /(https?:\/\/[^\s]+)|(www\.[^\s]+)|([^\s]+\.(com|org|edu|net))/i;
...
export const linkify = (text: string): React.ReactNode => {
  ...
  const urlRegex =
    /(https?:\/\/[^\s]+)|(www\.[^\s]+)|([^\s]+\.(com|org|edu|net)[^\s]*)/gi;
 ...
 */
export const formatComplexDateString = (
  dateString: string
): React.ReactNode => {
  ...
  return (
    <div className={cn('flex items-start justify-between mb-2', className)}>
      ...
      <div className="ml-4 font-medium text-right">
        {isApproximateDate && (
          <span className="text-gray-500 text-sm mr-1">Approx.</span>
        )}
...
export const GradeItem: React.FC<GradeItemProps> = ({
  ...
  // Clean up the weight display by removing any extra % signs
  ...
  return (
    <div className={cn('flex justify-between items-center mb-2', className)}>
      ...
      {displayWeight && (
        <div className="font-medium text-gray-400 ml-2">{displayWeight}</div>
      )}
    </div>
  );
};
